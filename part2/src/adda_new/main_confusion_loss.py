...
def train(config, encoder, discriminator, optimizer1, optimizer2, src_data_loader, tgt_data_loader, src_i2q, tgt_i2q, delta_lr):
    ...
    for batch_idx, (src_batch, tgt_batch) in tqdm(enumerate(combined_data_loader), desc="Training"):
        ...
        fake_src_target = torch.autograd.Variable(torch.ones((src_num)).long())  # domain 0
        fake_tgt_target = torch.autograd.Variable(torch.zeros((batch_size2)).long())
        if config.use_cuda:
            fake_src_target = fake_src_target.cuda()
            fake_tgt_target = fake_tgt_target.cuda()
        loss3 = discriminator.loss(src_emb, fake_src_target, False) \
                + discriminator.loss(tgt_emb, fake_tgt_target, False)
        ...
        loss_1_2_3 = loss1 + delta_lr * loss3 * 0.5 + delta_lr * loss2 * 0.5
        loss_1_2_3.backward(retain_graph=True)

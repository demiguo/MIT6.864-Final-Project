...
def train(config, src_encoder, tgt_encoder, discriminator, optimizer1, optimizer2, optimizer3, \
        src_data_loader, tgt_data_loader, src_i2q, tgt_i2q, delta_lr, train_discriminator=True):
    src_encoder.train()
    tgt_encoder.train()
    discriminator.train()
    ...
    for batch_idx, (src_batch, tgt_batch) in tqdm(enumerate(combined_data_loader), desc="Training"):
        ...
        q1_emb = 0.5 * (tgt_encoder(src_encoder(q1_title, q1_title_len)) + tgt_encoder(src_encoder(q1_body, q1_body_len)))
        ...
        # Train src_encoder and tgt encoder
...
def evaluate_for_android(src_encoder, tgt_encoder, data_loader, i2q):
    ...
    for batch_idx, (q1_ids, q2_ids, labels) in tqdm(enumerate(data_loader), desc="Evaluate"):
        ...
        q2_emb = 0.5 * (tgt_encoder(src_encoder(q2_title, q2_title_len))+ tgt_encoder(src_encoder(q2_body, q2_body_len)))
...
if __name__ == "__main__":
    ...
    tgt_encoder = myMAP(config)   # on top of src_encoder
    ...
    for epoch in tqdm(range(config.args.epochs), desc="Running"):
        ...
        src_encoder, tgt_encoder, discriminator, optimizer1, optimizer2, optimizer3, avg_loss, avg_acc = \
                ...
                src_train_loader, tgt_train_loader, src_i2q, tgt_i2q, delta_lr, train_discriminator)
        ...
        dev_auc = evaluate_for_android(src_encoder, tgt_encoder, dev_loader, tgt_i2q)
        test_auc = evaluate_for_android(src_encoder, tgt_encoder, test_loader, tgt_i2q)
        ...
        def save_checkpoint():
            checkpoint = {"src_encoder": src_encoder.state_dict(), 
                          ...
                          "args":config.args}
            checkpoint_file = "%s-domain-adapt-extend-epoch%d" % (config.args.model_file, epoch)
            config.log.info("=> saving checkpoint @ epoch %d to %s" % (epoch, checkpoint_file))
            torch.save(checkpoint, checkpoint_file)
        save_checkpoint()

    config.log.info("=> Best Model: Dev AUC %.3lf || Test AUC %.3lf || Saved at %s " % (best_dev_auc, best_test_auc, "%s-domain-adapt-extend-epoch%d" % (config.args.model_file, epoch)))

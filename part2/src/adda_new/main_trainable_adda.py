...
if __name__ == "__main__":
    ...
    for epoch in tqdm(range(config.args.epochs), desc="Running"):
        train_discriminator = True if epoch != 0 else False
        ...
        # NOTE: It seems that tgt_encoder cannot be trained from scratch
        if epoch == 0:
            '''
            import copy
            tgt_encoder.load_state_dict(copy.deepcopy(src_encoder.state_dict()))
            '''
            src_weights = list(src_encoder.parameters())
            tgt_weights = list(tgt_encoder.parameters())
            for i in range(len(src_weights)):
                tgt_weights[i].data[:] = src_weights[i].data[:]

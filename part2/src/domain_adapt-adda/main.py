...
def train(config, src_encoder, tgt_encoder, discriminator, encoder_optimizer, discriminator_optimizer, \
    ...
    for batch_idx, ((src_qid), (tgt_qid)) in tqdm(enumerate(zipped_data_loader), desc="Training"):
        ...
        domain_label_src = autograd.Variable(torch.zeros(src_q_emb.size(0)).long())
        domain_label_tgt = autograd.Variable(torch.ones(tgt_q_emb.size(0)).long())
        if config.use_cuda:
        ...
        fake_label_tgt = autograd.Variable(torch.zeros(tgt_q_emb.size(0)).long())
        if config.use_cuda:
        ...
        if ((batch_idx + 1) % config.args.log_step == 0):
            config.log.info('Iter: {}/{}, dis_loss: {:.3f}, enc_loss: {:.3f}, dis_acc: {:.3f}'
...
if __name__ == "__main__":
    ...
    encoder_optimizer = optim.Adam(tgt_encoder.get_train_parameters(), lr=1e-3 * 1e-2, weight_decay=1e-8 * 1e-2)
